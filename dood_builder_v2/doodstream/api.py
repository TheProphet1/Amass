...
class DoodStream:
    ...
    def search_videos(self, search_keyword):
        ...
        log.warning(url)
        return self.req(url)
    def list_videos(self,page,f_id):
        url = f"{self.base_url}file/list?key={self.api_key}&page={page}"
        
        return self.req(url)
    def list_folders_new(self,f_id):
        url=f"{self.base_url}file/info?key={self.api_key}&code={f_id}"
        return self.req(url)
    def list_folders(self,page,f_id):
        url = f"{self.base_url}folder/list?key={self.api_key}&page={page}&fld_id={f_id}"
        return self.req(url)
    def list_all_folders(self,f_id):
        url = f"{self.base_url}folder/list?key={self.api_key}&per_page=1000&file_code={f_id}"
        return self.req(url)
